...
public class BFReader {
	...
	public BFReader(String isbn) {
		...
		try {
			...
			Object obj = null;
			try {
				obj = parser.parse(new FileReader("c:\\wroehrl.json"));
...
/*
		InputStream is = null;
		try {
			is = new FileInputStream("C:\\JSON.json");
...
/*		try {
			bfr = new BufferedReader(new InputStreamReader(new URL(url+isbn).openStream()));
			Leser l = new Leser(isbn);
